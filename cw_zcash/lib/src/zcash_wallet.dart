...
abstract class ZcashWalletBase
  ...
  Future<PendingTransaction> _createTransaction(
    ...
    final sendAmount = creds.outputs
        .map((final out) => out.formattedCryptoAmount ?? 0)
  ...
  ZcashTransactionInfo _zcashInfoFromMempoolTx(
    ...
    final netValue = accountNotes.fold<int>(
      0,
      (final sum, final note) => sum + BigInt.from(note.value).toInt(),
    );
    final direction = netValue >= 0
